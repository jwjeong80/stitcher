...
uint32_t CBitStreamWriter::write_sequence_header_obu(uint8_t *const dst) {
	CBitWriter wb(dst, 0);
 ...
 void CBitStreamWriter::write_profile(BITSTREAM_PROFILE profile) {
	assert(profile >= PROFILE_0 && profile < MAX_PROFILES);
	aom_wb_write_literal(profile, PROFILE_BITS);
}
