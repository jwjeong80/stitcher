...
aom_codec_err_t aom_read_obu_header(uint8_t *buffer, size_t buffer_length,
  ...
  struct aom_read_bit_buffer rb = { buffer, buffer + buffer_length, 0, NULL,
                                    NULL };
  aom_codec_err_t parse_result = read_obu_header(&rb, is_annexb, header); //obu.c
...
static uint32_t read_sequence_header_obu(AV1Decoder *pbi,
  ...
  seq_params->profile = av1_read_profile(rb); //decode_frame.c
  ...
  if (seq_params->reduced_still_picture_hdr) {
    ...
    if (cm->timing_info_present) {
      av1_read_timing_info_header(cm, rb); //decodeframe.c

      seq_params->decoder_model_info_present_flag = aom_rb_read_bit(rb);
      if (seq_params->decoder_model_info_present_flag)
        av1_read_decoder_model_info(cm, rb); //decodeframe.c
    } else {
      seq_params->decoder_model_info_present_flag = 0;
    }
...
static uint32_t read_frame_header_obu(AV1Decoder *pbi,
  ...
  return av1_decode_frame_headers_and_setup(pbi, rb, data, p_data_end,  //decodeframe.c
...
static uint32_t read_one_tile_group_obu(
  ...
  av1_decode_tg_tiles_and_wrapup(pbi, data, data_end, p_data_end, start_tile,
                                 end_tile, is_first_tg);  //decodeframe.c
...
static uint32_t read_and_decode_one_tile_list(AV1Decoder *pbi,
  ...
  alloc_tile_list_buffer(pbi); //obu.c
  ...
  for (i = 0; i <= pbi->tile_count_minus_1; i++) {
    ...
    av1_set_reference_dec(cm, 0, 1, &pbi->ext_refs.refs[ref_idx]); //decoder.c
    ...
    av1_decode_tg_tiles_and_wrapup(pbi, data, data + pbi->coded_tile_data_size, //decodeframe.c
    ...
    copy_decoded_tile_to_tile_list_buffer(pbi, &output); //obu.c
  }
...
static void read_metadata_scalability(const uint8_t *data, size_t sz) {
  ...
  if (scalability_mode_idc == SCALABILITY_SS) {
    scalability_structure(&rb); //obu.c
  }
}
...
static size_t read_metadata(const uint8_t *data, size_t sz) {
  ...
  if (aom_uleb_decode(data, sz, &type_value, &type_length) < 0) { //aom_integer.h
    return sz;
  }
...
static aom_codec_err_t read_obu_size(const uint8_t *data,
  ...
  if (aom_uleb_decode(data, bytes_available, &u_obu_size, length_field_size) !=  //aom_integer.c
      0) {
    return AOM_CODEC_CORRUPT_FRAME;
  }
...
aom_codec_err_t aom_read_obu_header_and_size(const uint8_t *data,
  ...
  status = read_obu_header(&rb, is_annexb, obu_header); //obu.c
  if (status != AOM_CODEC_OK) return status;
  ...
  while (!frame_decoding_finished && !cm->error.error_code) {
    ...
    aom_codec_err_t status =
        aom_read_obu_header_and_size(data, bytes_available, cm->is_annexb,
                                     &obu_header, &payload_size, &bytes_read); //obu.c
    ...
    av1_init_read_bit_buffer(pbi, &rb, data, data + payload_size); //decodeframe.c

    switch (obu_header.type) {
      case OBU_TEMPORAL_DELIMITER:
        decoded_payload_size = read_temporal_delimiter_obu(); //obu.c
        pbi->seen_frame_header = 0;
        break;
      case OBU_SEQUENCE_HEADER:
        if (!seq_header_received) {
          decoded_payload_size = read_sequence_header_obu(pbi, &rb); //obu.c
      ...
      case OBU_TILE_GROUP:
        ...
        decoded_payload_size += read_one_tile_group_obu(               //obu.c
      ...
      case OBU_TILE_LIST:
        ...
        decoded_payload_size =
            read_and_decode_one_tile_list(pbi, &rb, data, data + payload_size,  //decodeframe.c
